...
az group create --name jomitsfnew --location westus

az group deployment create --name initial-5-nodes --resource-group jomitsfnew --template-file template.json --parameters parameters.json

# To SCALE UP Primary VMSS -> Change the capacity in the VMSS and and run the deployment again

az group deployment create --name scale-9-nodes --resource-group jomitsfnew --template-file template.json --parameters parameters.json
...
az group deployment create --name scale-8-nodes --resource-group jomitsfnew --template-file template.json --parameters parameters.json
